...
const Strings = () => {
  ...
  const fetchQuestions = async () => {
    try {
      const response = await httpInjectorService.fetchQuestionsTopicWise({ topicId: 1 });
  ...
  const fetchSolvedQuestions = async () => {
    try {
      const response = await httpInjectorService.fetchSolvedQuestions({ userId, topicId: 1 });
      if (response.status === 'success') {
  ...
  const handleSolvedToggle = async (questionId) => {
    ...
    try {
      if (!isSolved) {
        await httpInjectorService.addSolvedQuestion({ userId, questionId, topicId: 1 });
