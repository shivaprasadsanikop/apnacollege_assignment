...
const Hashmap = () => {
  ...
  const fetchQuestions = async () => {
    try {
      const response = await httpInjectorService.fetchQuestionsTopicWise({ topicId: 6 });
  ...
  const fetchSolvedQuestions = async () => {
    try {
      const response = await httpInjectorService.fetchSolvedQuestions({ userId, topicId: 6 });
      if (response.status === 'success') {
  ...
  const handleSolvedToggle = async (questionId) => {
    ...
    try {
      if (!isSolved) {
        await httpInjectorService.addSolvedQuestion({ userId, questionId, topicId: 6 });
  ...
  return (
    ...
    <h4 className="mb-0 me-2">HashMap Questions</h4>
